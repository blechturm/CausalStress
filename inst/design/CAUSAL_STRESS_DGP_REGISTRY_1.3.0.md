...
4.  **Outcomes:** For all synthetic (non-placebo) DGPs, potential outcomes are generated via the additive model:
...
### 2.1 `synth_baseline`
**Goal:** Sanity check. The "Happy Path."
...
Strictly enforces **Sharp Null**: $Y_1 \equiv Y_0$.
*Implementation:* $\tau(X) \equiv 0$ and $Y_1$ is a pathwise copy of $Y_0$. No independent noise draw for treatment.
...
For these IDs, `true_att` may be `NA`:
* **`twins` / `401k`:** Used for relative comparison only.
...
```r
list(
  # The set of covariates in df MUST match the DGP definition
  df = tibble(y, w, y0, y1, p, x1, ...), # y0, y1, p REQUIRED
  true_att = <numeric>,                  # REQUIRED
  true_qst = <tibble>,                   # REQUIRED
                                         # Columns: 'tau' (numeric u), 'value' (numeric QST)
  meta = list(
    type = "synthetic",                  # REQUIRED
    structural_te = <numeric vector>,    # REQUIRED
    ...
  )
)