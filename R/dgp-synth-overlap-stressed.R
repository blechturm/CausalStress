...
#' @export
dgp_synth_overlap_stressed_v130 <- function(n, seed = NULL, include_truth = TRUE, oracle_only = FALSE) {
  if (!is.null(seed)) {
    cs_set_rng(seed)
  }
...
#'
#' This variant clones the v1.4.0 (hardened) propensity.
#'
#' @inheritParams dgp_synth_overlap_stressed_v130
#'
#' @return A synthetic DGP list satisfying the contract.
#' @export
dgp_synth_overlap_stressed_v160 <- function(n, seed = NULL, include_truth = TRUE, oracle_only = FALSE) {
  if (!is.null(seed)) {
    cs_set_rng(seed)
  }
  ...
  true_att <- cs_true_att(structural_te = tau, w = w)
  true_qst <- if (isTRUE(include_truth)) {
    cs_get_oracle_qst("synth_overlap_stressed", version = "1.6.0")
  } else {
    tibble::tibble(tau = cs_tau_oracle, value = rep(NA_real_, length(cs_tau_oracle)))
  }
...
dgp_synth_overlap_stressed <- function(n, seed = NULL) {
  dgp_synth_overlap_stressed_v160(n = n, seed = seed)
}
