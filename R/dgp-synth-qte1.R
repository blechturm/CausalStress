...
#'
#' @inheritParams dgp_synth_qte1_v130
#'
#' @return A synthetic DGP list satisfying the contract.
#' @export
dgp_synth_qte1_v160 <- function(n, seed = NULL, include_truth = TRUE, oracle_only = FALSE) {
  if (!is.null(seed)) {
    cs_set_rng(seed)
  }
  ...
  true_att <- cs_true_att(structural_te = tau, w = w)
  true_qst <- if (isTRUE(include_truth)) cs_get_oracle_qst("synth_qte1", version = "1.6.0") else NULL
