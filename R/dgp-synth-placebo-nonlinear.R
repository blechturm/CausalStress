...
#' @export
dgp_synth_placebo_nonlinear_v130 <- function(n, seed = NULL, include_truth = TRUE, oracle_only = FALSE) {
  if (!is.null(seed)) {
    cs_set_rng(seed)
  }
  ...
  true_qst <- if (isTRUE(include_truth)) cs_get_oracle_qst("synth_placebo_nonlinear", version = "1.3.0") else NULL
