...
First, we load the package and set up a **Board**. In `CausalStress`, results are never just "returned" to the console—they are persisted to a storage backend (local folder, S3, etc.).
...
system.time({
  runs_resume <- cs_run_grid(
    ...
    n             = 500,
    seeds         = 1:5,
    bootstrap     = TRUE,   # match original config
    B             = 100,    # match original config
    board         = board,
    skip_existing = TRUE
  )
})
