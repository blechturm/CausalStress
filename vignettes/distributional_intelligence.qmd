...
run_optional_gengc <- requireNamespace("GenGC", quietly = TRUE) &&
  identical(Sys.getenv("CAUSALSTRESS_RUN_OPTIONAL_DOCS"), "true")
```
...
```{r run-gengc, eval=run_optional_gengc}
...
## The numbers
```{r compare-att, eval=run_optional_gengc}
...
```{r plot-qst, eval=run_optional_gengc}
