...
test_that("synth_nonlinear_heteroskedastic exhibits increasing noise in X2 tails", {
  dgp <- dgp_synth_nonlinear_heteroskedastic(n = 5000, seed = 1)
  df  <- dgp$df

  mu0_hat <- 1 + 0.5 * df$X1^3 + 1.5 * df$X2^2 - 1.0 * df$X4
  res     <- df$y0 - mu0_hat
